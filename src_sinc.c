...
static void calc_output (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double* output) ;
static void calc_output_2ch (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double* output);
static void calc_output_6ch (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double* output);
...
{	SINC_FILTER *filter ;
	...
	double outp_temp[filter->channels];
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		//printf("chans=%d\n",filter->channels);
		if(filter->channels==6){
		    calc_output_6ch (filter, increment, start_filter_index, outp_temp);
		    data->data_out[filter->out_gen+0] = (float) ( (float_increment / filter->index_inc) * outp_temp[0]);
		    data->data_out[filter->out_gen+1] = (float) ( (float_increment / filter->index_inc) * outp_temp[1]);
		    data->data_out[filter->out_gen+2] = (float) ( (float_increment / filter->index_inc) * outp_temp[2]);
		    data->data_out[filter->out_gen+3] = (float) ( (float_increment / filter->index_inc) * outp_temp[3]);
		    data->data_out[filter->out_gen+4] = (float) ( (float_increment / filter->index_inc) * outp_temp[4]);
		    data->data_out[filter->out_gen+5] = (float) ( (float_increment / filter->index_inc) * outp_temp[5]);
		    filter->out_gen+=6;
		} else
		if(filter->channels==2){
		    calc_output_2ch (filter, increment, start_filter_index, outp_temp);
		    data->data_out[filter->out_gen+0] = (float) ( (float_increment / filter->index_inc) * outp_temp[0]);
		    data->data_out[filter->out_gen+1] = (float) ( (float_increment / filter->index_inc) * outp_temp[1]);
		    filter->out_gen+=2;
		} else
		{
		    calc_output (filter, increment, start_filter_index, outp_temp);
		    for (ch = 0 ; ch < filter->channels ; ch++){
			data->data_out [filter->out_gen] = (float) (
			    (float_increment / filter->index_inc) * outp_temp[ch]
			) ;
			filter->out_gen ++ ;
		    }
		}
...
static void
calc_output_6ch (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double* output)
{	double		fraction, icoeff ;
	...
	{	fraction = fp_to_double (filter_index) ;
		...
		output[0] += icoeff * filter->buffer[data_index+0];
		output[1] += icoeff * filter->buffer[data_index+1];
		output[2] += icoeff * filter->buffer[data_index+2];
		output[3] += icoeff * filter->buffer[data_index+3];
		output[4] += icoeff * filter->buffer[data_index+4];
		output[5] += icoeff * filter->buffer[data_index+5];
...
static void
calc_output_2ch (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double* output)
{	double		fraction, icoeff ;
	increment_t	filter_index, max_filter_index ;
	int		data_index, coeff_count, indx ;
...
static void
calc_output (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double* output)
{	double		fraction, icoeff ;
	increment_t	filter_index, max_filter_index ;
	int		i, data_index, coeff_count, indx ;

	for(i=0;i<filter->channels;i++) output[i]=0.0;
	...
	{	fraction = fp_to_double (filter_index) ;
		...
		for(i=0;i<filter->channels;i++)
		    output[i] += icoeff * filter->buffer[data_index+i];
